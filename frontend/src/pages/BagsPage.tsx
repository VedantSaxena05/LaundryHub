...
export default function BagsPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="My Bags"
        description="Track your laundry bags"
      ...
      {bag ? (
        <AnimatedCard delay={0.1} className="border-l-4 border-l-primary">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 font-display"><Package className="h-5 w-5 text-primary" /> Current Bag</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Date: {bag.date}</p>
                {bag.tag_uid && <p className="text-sm text-muted-foreground">Tag: {bag.tag_uid}</p>}
              </div>
        ...
        <AnimatedCard delay={0.1}>
          <CardContent className="py-12 text-center text-muted-foreground font-medium">No active bag</CardContent>
        </AnimatedCard>
      )}

      {history.length > 0 && (
        <AnimatedCard delay={0.2}>
          <CardHeader><CardTitle className="flex items-center gap-2 font-display text-lg"><History className="h-5 w-5 text-primary" /> History</CardTitle></CardHeader>
          <CardContent>
      ...
      {selectedBag && logs.length > 0 && (
        <AnimatedCard delay={0.3}>
          <CardHeader><CardTitle className="font-display text-lg">Status Log</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-3">
              {logs.map(l => (
