...
export default function ForumPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Forum"
        description="Discuss, share tips, and announcements"
          ...
          <Dialog open={showCreate} onOpenChange={setShowCreate}>
            ...
            <DialogContent className="rounded-2xl">
              ...
              <div className="space-y-4">
                ...
                <Button onClick={handleCreate} className="w-full rounded-xl font-semibold h-11">Post</Button>
              </div>
            </DialogContent>
          </Dialog>
        }
      ...
      <div className="space-y-4">
            ...
            <CardContent className="pt-5">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    {p.is_anonymous && <Badge variant="secondary" className="text-xs rounded-lg">Anonymous</Badge>}
                  </div>
              ...
              {replyTo === p.id && (
                <div className="mt-4 space-y-3 p-4 bg-accent/30 rounded-xl border border-border/50">
                  <Textarea value={replyBody} onChange={e => setReplyBody(e.target.value)} placeholder="Write a reply..." rows={2} className="rounded-xl" />
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Switch checked={replyAnon} onCheckedChange={setReplyAnon} />
                      <Label className="text-xs font-medium">Anonymous</Label>
                    </div>
                    <Button size="sm" onClick={handleReply} className="rounded-xl"><Send className="h-3.5 w-3.5 mr-1.5" /> Reply</Button>
                  </div>
                </div>
              )}
            </CardContent>
        ...
        {posts.length === 0 && <p className="text-center text-muted-foreground py-12 font-medium">No posts yet</p>}
      </div>
    </div>
  );
}
