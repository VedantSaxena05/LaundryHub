...
export default function BlockedDatesPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Blocked Dates"
        description="Manage dates when laundry service is unavailable"
          ...
          <Dialog open={showCreate} onOpenChange={setShowCreate}>
            <DialogTrigger asChild>
              <Button className="rounded-xl font-semibold gradient-primary border-0 text-primary-foreground hover:opacity-90">
                <Plus className="h-4 w-4 mr-1.5" /> Block Date
              </Button>
            </DialogTrigger>
            <DialogContent className="rounded-2xl">
              ...
              <div className="space-y-4">
                ...
                <Button onClick={handleAdd} className="w-full rounded-xl h-11 font-semibold">Block Date</Button>
              </div>
            </DialogContent>
          </Dialog>
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {dates.length === 0 && <p className="text-center text-muted-foreground py-12 col-span-3 font-medium">No blocked dates</p>}
      </div>
    </div>
  );
}
