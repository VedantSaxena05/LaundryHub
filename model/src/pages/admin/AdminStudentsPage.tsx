...
export default function AdminStudentsPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={t("admin_students.title")}
        description={`${students.length} ${t("admin_students.registered")}`}
        icon={<Users className="w-6 h-6 text-primary-foreground" />}
      />

      <div className="relative opacity-0 animate-fade-in" style={{ animationDelay: '0.1s' }}>
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("admin_students.search_placeholder")} className="pl-11 h-12 rounded-xl" />
      </div>
            ...
            <CardContent className="pt-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={s.is_active ? "default" : "secondary"} className="rounded-lg">{s.is_active ? t("common.active") : t("common.inactive")}</Badge>
                  {s.is_active && (
