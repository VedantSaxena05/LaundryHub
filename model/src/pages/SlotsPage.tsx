  ...
  return (
    <div className="space-y-8">
      ...
      <AnimatedCard delay={0.1}>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 font-display"><Calendar className="h-5 w-5 text-primary" /> {t("slots.select_date")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label className="flex items-center gap-2 text-sm font-medium"><Clock className="h-4 w-4 text-muted-foreground" /> {t("slots.arrival_time")}</Label>
              ...
              <p className="text-xs text-muted-foreground">{t("slots.between")}</p>
            </div>
          </div>
          <div className="space-y-3">
            <Label className="flex items-center gap-2 text-sm font-medium">
              <Timer className="h-4 w-4 text-muted-foreground" /> {t("slots.window_duration")}: <span className="font-bold text-primary">{submissionWindow} min</span>
            </Label>
          ...
          <Button onClick={handleBook} disabled={loading} className="w-full h-12 rounded-xl font-semibold gradient-primary border-0 text-primary-foreground hover:opacity-90 transition-all">
            {loading ? t("slots.booking") : t("slots.book")}
          </Button>
        </CardContent>
      </AnimatedCard>
            ...
            <AnimatedCard key={b.block} delay={0.2 + i * 0.08} className="group hover:scale-[1.02]">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center justify-between font-display">
                  <span>{t("common.block")} {b.block}</span>
                  {b.is_available ? (
                    <Badge className="bg-success text-success-foreground rounded-lg">{t("slots.available")}</Badge>
                  ) : (
                    <Badge variant="destructive" className="rounded-lg">{t("slots.full")}</Badge>
                  )}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2.5 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground"><Clock className="h-3.5 w-3.5" /><span>{t("slots.dropoff")}: {b.dropoff_window}</span></div>
                <div className="flex items-center gap-2 text-muted-foreground"><Clock className="h-3.5 w-3.5" /><span>{t("slots.collection")}: {b.collection_window}</span></div>
                <div className="flex items-center justify-between pt-3 border-t border-border/50">
                  <span className="text-muted-foreground font-medium">{t("slots.remaining")}</span>
      ...
      {mySlots.length > 0 && (
        <AnimatedCard delay={0.5}>
          <CardHeader><CardTitle className="font-display text-lg">{t("slots.my_slots")}</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-3">
              {mySlots.map(s => (
                <div key={s.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-accent/30 border border-border/50 gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {s.submission_expires_at && s.status === "booked" && (
                        <p className="text-xs text-muted-foreground">{t("slots.dropoff_by")} {formatTime(s.submission_expires_at)}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {s.status === "booked" && (
                      <Button variant="ghost" size="sm" className="text-destructive rounded-xl" onClick={() => handleCancel(s.id)}>{t("common.cancel")}</Button>
                    )}
                  </div>
                </div>
              ))}
